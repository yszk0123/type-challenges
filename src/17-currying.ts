...
type Curry<Args, R> = Args extends [infer Head, ...infer Rest]
  ? (head: Head) => Curry<Rest, R>
  : R;
type CurryFn<Fn> = Fn extends () => unknown
  ? Fn
  : Fn extends (...args: infer Args) => infer R
    ? Curry<Args, R>
    : never;
declare function Currying<Fn>(fn: Fn): CurryFn<Fn>;
type a0 = Curry<[], boolean>;
type a = CurryFn<() => boolean>;
type b = CurryFn<(x: 1) => boolean>;
type c = CurryFn<(x: 1, y: 2) => boolean>;
type d = CurryFn<(x: 1, y: 2, z: 3) => boolean>;
